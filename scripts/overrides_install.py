...
def install_custom_configs():
    """Install macros.cfg, start_print.cfg, overrides.cfg, and overrides2.cfg to the custom config directory"""
    # Ensure custom directory exists
    os.makedirs(CUSTOM_CONFIG_DIR, exist_ok=True)
    ...
    install_map = [
        ...
        (REPO_ROOT / "configs" / "overrides2.cfg", Path(CUSTOM_CONFIG_DIR) / "overrides2.cfg", "overrides2.cfg"),
    ]
...
def update_custom_main_cfg(dry_run: bool = False) -> bool:
    ...
    include_regex = re.compile(r"^\s*\[include\s+(macros\.cfg|start_print\.cfg|overrides\.cfg|overrides2\.cfg)\s*\]\s*$", re.MULTILINE)
    ...
    if dry_run:
        if content == new_content:
            ...
            log("DRY RUN: Would update custom/main.cfg with ordered includes: macros, start_print, overrides, overrides2")
        return True
    ...
    try:
        with open(main_cfg_path, "w", encoding="utf-8") as f:
            f.write(new_content)
        log("Updated custom/main.cfg with ordered includes for macros, start_print, overrides, and overrides2")
        return True
    except Exception as e:
